    ...
    <div className='h-32 p-5 header-bg bg-slate-50 border-b border-slate-300' >
        ...
        <div className='flex space-x-3'>
            ...
            <div className="flex flex-col">
                <p className=' text-slate-700 font-semibold'>Hello, NEVILY WEB DEVELOPER</p>
        ...
        <nav className='sticky mt-6 flex space-x-4'>
            {
                navLinks.map((item, i)=>{
                    return(
                        <Link key={i} href={item.href} className={` ${pathname === item.href ? "py-1 border-b-4 border-blue-600": 'py-1'}`} >
                        {item.title}
                        </Link>
                    )
                })
